...
def yolo_to_bytetrack(input_label_dir, output_txt_path, image_width=1920, image_height=1080):
    ...
    with open(output_txt_path, "w") as out_file:
        for label_file in label_files:
            ...
            try:
                # M0101_img000001 → "000001" → frame_id
...
variants = ["blur", "hist_sharp", "bilateral_gamma", "raw"]
for variant in variants:
    input_dir = os.path.join(project_root, "yolo_output", variant, "labels")
    output_txt = os.path.join(project_root, "converted_outputs", f"{variant}.txt")
